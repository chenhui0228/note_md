...
from decimal import *
getcontext().prec = 6
Decimal(1)/Decimal(7) # Decimal('0.142857')，六个有效数字
</pre>
...
<pre>
from decimal import *
Decimal('50.5679').quantize(Decimal('0.00')) # Decimal('50.57')，结果四舍五入保留了两位小数
</pre>
...
<pre>
from decimal import *
str(Decimal('3.40').quantize(Decimal('0.0'))) # '3.40'，字符串类型
</pre>

### statistics
总体方差是我们学习时的标准方差，值是唯一的。而样本方差是从n个里最多选取n-1个进行计算，值有点随机性。只做为评估
...
求算术平均数,对分数Fraction及精度小数Decimal也适用
